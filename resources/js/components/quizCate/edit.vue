      ...
      <div class="row">
        <div class="col-12 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              <div class="row">
                <div class="col-md-3"><h4 class="card-title">Sửa môn học</h4></div>
...
export default {
  ...
  methods: {
    ...mapActions(["editQuizCategoryId","saveQuizCategory", "loadings","listQuizCategoryMain"]),
    nameImage(event) {
    ...
    saveEdit() {
      ...
      if (this.errors.length > 0) {
        ...
        this.loadings(true);
        this.saveQuizCategory(this.objData)
        .then(response => {
            this.loadings(false);
            this.$router.push({ name: "list_quiz_category" });
            this.$success("Sửa danh mục thành công");
            this.$route.push({ name: "list_quiz_category" });
          })
          .catch(error => {
            this.loadings(false);
    ...
    editQuizCategoryIds(){
      this.loadings(true);
      this.editQuizCategoryId(this.objData).then(response => {
        this.loadings(false);
