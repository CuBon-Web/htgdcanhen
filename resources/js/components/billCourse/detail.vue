<template>
          ...
          <div class="col-md-8 grid-margin stretch-card">
            <div class="card">
              <div class="card-body">
                ...
                <template slot="thead">
                  <vs-th>Tên khóa học</vs-th>
                  <vs-th>Giá bán</vs-th>
                </template>
                <template>
                  <vs-tr >
                    <vs-td>{{bill.product.name}}</vs-td>
                    <vs-td>{{formatNumber(bill.product.discount)}}đ</vs-td>
                    <vs-td v-if="bill.product.price == 0 && bill.product.discount == 0">Miễn phí</vs-td>
                  </vs-tr>
                </template>
              </vs-table>
                <div class="row">
  ...
  export default {
    ...
    watch: {},
    methods: {
      ...mapActions(["loadings","detailBillCourse","changeCourseStatus"]),
      closePop(event) {
        this.popupActivo = event;
      },
      changeUnPaymented(){
        this.loadings(true);
        this.changeCourseStatus({'status':0,'bill_id':this.$route.params.bill_id}).then(response => {
          this.loadings(false);
          this.detailBills();
          this.$success('Đã chuyển về cần kiểm tra');
          this.$router.push({ name: "billCoursePaymented" });
        })
      },
      changePaymented(){
        this.loadings(true);
        this.changeCourseStatus({'status':1,'bill_id':this.$route.params.bill_id}).then(response => {
          this.loadings(false);
          this.detailBills();
          this.$success('Xác nhận thành công');
          this.$router.push({ name: "billCourseDraft" });
        })
      },
      detailBills(){
        this.detailBillCourse(this.bill.bill_id).then(response => {
