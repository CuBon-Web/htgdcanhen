<template>
    <!-- partial -->
    <div>
        <h3 class="page-title">Đơn hàng khóa học đã thanh toán</h3>
        <div class="row">
                ...
                <vs-table max-items="5" pagination :data="list" v-if="list.length > 0">
                  ...
                  <template slot-scope="{data}">
                    <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                      ...
                      <vs-td :data="tr.created_at">{{(tr.product.name)}}</vs-td>
                      ...
                      <vs-td :data="tr.id">
                        <router-link :to="{name:'billCourserDetail',params:{bill_id:tr.bill_id}}">
                          <vs-button
                            vs-type="gradient"
                            size="lagre"
                            color="success"
  ...
  export default {
    ...
    methods: {
      ...mapActions(["listBillCourse","destroyCate", "loadings"]),
      closePop(event) {
        this.listCategory();
      ...
      listBillCourses() {
        this.loadings(true);
        this.listBillCourse({ keyword: this.keyword,status:1 })
        .then(response => {
            this.loadings(false);
