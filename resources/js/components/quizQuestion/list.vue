        ...
        <div class="col-md-12 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              ...
              <router-link class="nav-link" :to="{name:'add_quiz_question'}">
              ...
              <div class="row">
                ...
                <div class="col-md-2">
                  <router-link class="mb-0 font-weight-light" :to="{name:'questionImport'}">
              ...
              <vs-table max-items="20" pagination :data="list">
                ...
                <template slot-scope="{data}">
                  <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td :data="tr.id">
                      <router-link :to="{name:'edit_quiz_question',params:{id:tr.id}}">
                        <vs-button
                          vs-type="gradient"
                          size="lagre"
                          color="success" 
...
export default {
  ...
  methods: {
    ...mapActions(["listQuizQuestion","deleteQuizQuestionId", "loadings",'listQuizExam','findPartExam','findQuestionPartExam']),
    closePop(event) {
      this.listCategory();
      this.popupActivo = event;
    },
    findQuestionPartExample(){
      this.findPartExam(this.fillter.exam_id).then((response) => {
        this.listPart = response.data;
      });
      this.findQuestionPartExam(this.fillter).then((response) => {
    ...
    listCategory() {
      this.loadings(true);
      this.listQuizQuestion({ keyword: this.keyword })
      .then(response => {
          this.loadings(false);
          this.list = response.data;
        });
    ...
    destroy(){
      this.loadings(true);
      this.deleteQuizQuestionId({id:this.id_item})
      .then(response => {
        this.listCategory()
        this.loadings(false);
