<template>
          ...
          <div class="col-md-8 grid-margin stretch-card">
            <div class="card">
              <div class="card-body">
                ...
                <template slot="thead">
                  <vs-th>Tên đề thi</vs-th>
                  <vs-th>Giá bán</vs-th>
                </template>
                <template>
                  <vs-tr >
                    <vs-td>{{bill.dethi.title}}</vs-td>
                    <vs-td>{{formatNumber(bill.dethi.price)}}đ</vs-td>
                    <vs-td v-if="bill.dethi.price == 0">Miễn phí</vs-td>
                  </vs-tr>
                </template>
              </vs-table>
                <div class="row">
  ...
  export default {
    ...
    watch: {},
    methods: {
      ...mapActions(["loadings","detailBillDethi","changeDethiStatus"]),
      closePop(event) {
        this.popupActivo = event;
      },
      changeUnPaymented(){
        this.loadings(true);
        this.changeDethiStatus({'status':0,'bill_id':this.$route.params.bill_id}).then(response => {
          this.loadings(false);
          this.detailBills();
          this.$success('Đã chuyển về cần kiểm tra');
          this.$router.push({ name: "billDethiPaymented" });
        })
      },
      changePaymented(){
        this.loadings(true);
        this.changeDethiStatus({'status':1,'bill_id':this.$route.params.bill_id}).then(response => {
          this.loadings(false);
          this.detailBills();
          this.$success('Xác nhận thành công');
          this.$router.push({ name: "billDethiDraft" });
        })
      },
      detailBills(){
          this.detailBillDethi(this.bill.bill_id).then(response => {
