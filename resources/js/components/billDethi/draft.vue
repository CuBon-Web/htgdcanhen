<template>
    <!-- partial -->
    <div>
        <h3 class="page-title">Đơn hàng đề thi cần kiểm tra</h3>
        <div class="row">
                ...
                <div class="row" v-if="list.length == 0">
                  ...
                  <div class="col-md-4">
                    <div class="mb-0 flex-grow">
                      ...
                      <p class="mb-0 font-weight-light">Đơn hàng cần kiểm tra là đơn hàng mà<br> đang ở trạng thái kiểm tra đăng ký mua đề thi<br></p>
                    </div>
                  </div>
                  <div class="col-md-4">
                ...
                <vs-table max-items="5" pagination :data="list" v-if="list.length > 0">
                  ...
                  <template slot-scope="{data}">
                    <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                      ...
                      <vs-td :data="tr.created_at">{{(tr.dethi.title)}}</vs-td>
                      ...
                      <vs-td :data="tr.id">
                        <router-link :to="{name:'billDethiDetail',params:{bill_id:tr.bill_id}}">
                          <vs-button
                            vs-type="gradient"
                            size="lagre"
                            color="success"
  ...
  export default {
    ...
    methods: {
      ...mapActions(["listBillDethi","destroyCate", "loadings"]),
      closePop(event) {
        this.listCategory();
      ...
      listBillDethis() {
        this.loadings(true);
        this.listBillDethi({ keyword: this.keyword,status:0 })
        .then(response => {
            this.loadings(false);
