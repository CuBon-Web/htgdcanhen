        ...
        <div class="col-md-12 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              ...
              <router-link class="nav-link" :to="{name:'addDocument'}">
              ...
              <vs-table stripe :data="list" max-items="20" pagination>
                ...
                <template slot-scope="{data}">
                  <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td :data="tr.name">{{tr.cate.name}}</vs-td>
                    <vs-td >
                      <router-link :to="{name:'editDocument',params:{id:tr.id}}">
                        <vs-button
                          vs-type="gradient"
                          size="lagre"
                          color="success"
...
export default {
  ...
  watch: {},
  methods: {
    ...mapActions(['listDocument','loadings','deleteDocument']),
    listDocuments(){
      this.listDocument({ keyword: this.keyword })
      .then(response => {
          this.loadings(false);
    ...
    destroy(){
      this.deleteDocument({id:this.id_item}).then(response => {
        this.listDocuments();
        this.loadings(false);
