      ...
      <div class="row fixxed">
        <div class="col-12">
          <div class="saveButton">
            <vs-button color="primary" @click="addDocuments">Cập nhật</vs-button>
          </div>
        </div>
      </div>
...
export default {
  ...
  watch: {},
  methods: {
    ...mapActions(["addDocument", "loadings","detailDocument","listLanguage","listCateDocument"]),
    addDocuments(){
      this.errors = [];
      ...
      if (this.errors.length > 0) {
        ...
        this.loadings(true);
        this.addDocument(this.objData).then(response => {
          this.loadings(false);
    ...
    editById() {
      this.loadings(true);
      this.detailDocument(this.objData).then(response => {
        this.loadings(false);
