        ...
        <div class="col-md-12 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              ...
              <router-link class="nav-link" :to="{name:'add_quiz_question_group'}">
              ...
              <vs-table max-items="20" pagination :data="list">
                ...
                <template slot-scope="{data}">
                  <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td :data="tr.id">
                      <router-link :to="{name:'edit_quiz_question_group',params:{id:tr.id}}">
                        <vs-button
                          vs-type="gradient"
                          size="lagre"
                          color="success" 
...
export default {
  ...
  methods: {
    ...mapActions(["listQuizQuestionGroup","deleteQuizQuestionIdGroup", "loadings",'listQuizExam','findGroupQuestion']),
    closePop(event) {
      this.listCategory();
      this.popupActivo = event;
    },
    findGroupExam(){
      this.findGroupQuestion({exam_id:this.exam_id}).then((response) => {
    ...
    listCategory() {
      this.loadings(true);
      this.listQuizQuestionGroup({ keyword: this.keyword })
      .then(response => {
          this.loadings(false);
          this.list = response.data;
        });
    ...
    destroy(){
      this.loadings(true);
      this.deleteQuizQuestionIdGroup({id:this.id_item})
      .then(response => {
        this.listCategory()
        this.loadings(false);
