<template>
  <!-- partial -->
  <div>
      <h3 class="page-title">Đơn hàng Tài liệu đã thanh toán</h3>
      <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              ...
              <div class="row" v-if="list.length == 0">
                ...
                <div class="col-md-4">
                  <div class="mb-0 flex-grow">
                    ...
                    <p class="mb-0 font-weight-light">Đơn hàng cần kiểm tra là đơn hàng mà<br> đang ở trạng thái đã thanh toán<br></p>
                  </div>
                </div>
                <div class="col-md-4">
              ...
              />
              <vs-table max-items="20" pagination :data="list" v-if="list.length > 0">
                <template slot="thead">
                  <vs-th>Mã đơn hàng</vs-th>
                  <vs-th>Tài liệu</vs-th>
                ...
                <template slot-scope="{data}">
                  <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                    <vs-td :data="tr.id">#{{tr.bill_id}}</vs-td>
                    <vs-td :data="tr.created_at">{{(tr.document.name)}}</vs-td>
                    <vs-td :data="tr.customer">
                      {{tr.customer ? tr.customer.name : "Vô danh"}}
                    </vs-td>
                    <vs-td :data="tr.id">
                        <vs-chip color="primary" v-if="tr.status == 0">
                          Đợi kiểm tra
                        </vs-chip>
                    </vs-td>
                    <vs-td :data="tr.id">
                      <router-link :to="{name:'billDocumentDetail',params:{bill_id:tr.bill_id}}">
                        <vs-button
                          vs-type="gradient"
                          size="lagre"
                          color="success"
...
export default {
  ...
  methods: {
    ...mapActions(["listBillDocument","destroyCate", "loadings"]),
    closePop(event) {
      this.listCategory();
    ...
    listBillDocuments() {
      this.loadings(true);
      this.listBillDocument({ keyword: this.keyword,status:1 })
      .then(response => {
          this.loadings(false);
