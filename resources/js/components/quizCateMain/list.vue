  ...
  <div>
      <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">Khối lớp</h4>
              <router-link class="nav-link" :to="{name:'add_quiz_category_main'}">
                ...
                <template slot-scope="{data}">
                  <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td :data="tr.id">
                      ...
                      <router-link :to="{name:'edit_quiz_category_main',params:{id:tr.id}}">
                        <vs-button
                          vs-type="gradient"
                          size="lagre"
                          color="success" 
...
export default {
  ...
  methods: {
    ...mapActions(["listQuizCategoryMain","deleteQuizCategoryIdMain", "loadings"]),
    closePop(event) {
      this.listCategory();
    ...
    listCategory() {
      this.loadings(true);
      this.listQuizCategoryMain({ keyword: this.keyword })
      .then(response => {
          this.loadings(false);
    ...
    destroy(){
      this.loadings(true);
      this.deleteQuizCategoryIdMain({id:this.id_item})
      .then(response => {
        this.listCategory()
        this.loadings(false);
