...
export default {
  ...
  methods: {
    ...mapActions(["saveDocsPart","listDocsExam", "loadings","listDocsCategory","editDocsPartId"]),
    nameImage(event) {
    ...
    saveEdit() {
      ...
      if (this.errors.length > 0) {
        ...
        this.loadings(true);
        this.saveDocsPart(this.objData)
        .then(response => {
            this.loadings(false);
            this.$router.push({ name: "list_docs_part" });
            this.$success("Sửa thành công");
            this.$route.push({ name: "list_docs_part" });
          })
          .catch(error => {
            this.loadings(false);
    ...
    editDocsPartIds(){
      this.loadings(true);
      this.editDocsPartId(this.objData).then(response => {
        this.loadings(false);
