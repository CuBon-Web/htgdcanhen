  ...
  <div>
      <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">Danh mục học liệu</h4>
              <router-link class="nav-link" :to="{name:'add_docs_category'}">
                ...
                <template slot-scope="{data}">
                  <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td :data="tr.id">
                      ...
                      <router-link :to="{name:'edit_docs_category',params:{id:tr.id}}">
                        <vs-button
                          vs-type="gradient"
                          size="lagre"
                          color="success" 
...
export default {
  ...
  methods: {
    ...mapActions(["listDocsCategory","deleteDocsCategoryId", "loadings"]),
    closePop(event) {
      this.listCategory();
    ...
    listCategory() {
      this.loadings(true);
      this.listDocsCategory({ keyword: this.keyword })
      .then(response => {
          this.loadings(false);
